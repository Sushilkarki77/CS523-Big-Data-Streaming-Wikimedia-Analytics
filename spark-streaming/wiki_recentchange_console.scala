...
val checkpointRoot = sys.env.getOrElse(
  ...
  "hdfs://localhost:9000/tmp/wiki-pulse/checkpoints/console"
)
...
println(
  s"""
     |Starting Wikimedia recentchange Structured Streaming job
...
val throughput = watermarkedEvents
  ...
  .select(
    ...
    col("edit_count"),
    col("bot_edit_count"),
    current_timestamp().as("batch_written_at")
  )
...
def printBatch(label: String, sortColumns: Seq[org.apache.spark.sql.Column])(
    batchDF: DataFrame,
    batchId: Long
): Unit = {
  println(s"\n===== $label: batch $batchId =====")
  val sorted = if (sortColumns.nonEmpty) batchDF.orderBy(sortColumns: _*) else batchDF
  sorted.show(20, truncate = false)
}

val throughputQuery = throughput.writeStream
  .queryName("wiki_pulse_throughput_console")
  .outputMode("update")
  .option("checkpointLocation", s"$checkpointRoot/throughput")
  .trigger(Trigger.ProcessingTime(triggerInterval))
  .foreachBatch { (batchDF: DataFrame, batchId: Long) =>
    printBatch("throughput", Seq(col("window_start").desc))(batchDF, batchId)
  }
  .start()

val byWikiQuery = byWiki.writeStream
  .queryName("wiki_pulse_by_wiki_console")
  .outputMode("update")
  .option("checkpointLocation", s"$checkpointRoot/by-wiki")
  .trigger(Trigger.ProcessingTime(triggerInterval))
  .foreachBatch { (batchDF: DataFrame, batchId: Long) =>
    printBatch("by_wiki_top_counts", Seq(col("window_start").desc, col("edit_count").desc))(batchDF, batchId)
  }
  .start()
